...
class TarifaService
{
    ...
    public function unTramo($fields, $startDate, $endDate, $days, $freeDay=null, $tipo_tarifa_id=null)
    {
        ...
        //asigna el rango dependiendo del número de dias y la var freeDay
        if (in_array($freeDay, $rangeDays)) {
            foreach ($rangeDays as $key => $day) {
                if ($days < $day) {
                    $desde = $rangeDays[$key - 1];
                    break;
        ...
        foreach ($tipos as $tipo) {
            ...
            foreach ($searchTarifas as $key => $value) {
               ...
               elseif ($key == count($searchTarifas) - 1) {
                    ...
                    $tarifa_importe_vehiculo += $value->eur_dia * $days; 
                    if($startDateFull->format('Y-m-d') == date('Y').'-12-31'){
                        $days += 1;
                    }   
                    if($startDateFull->format('Y-m-d') == date('Y').'-12-30'){
                        $days += 2;
                    }
    ...
    public function getTarifa($fields)
    {
        ...
        $tarifas = $this->unTramo($fields, $startDate, $endDate, $daysDate,null,null);
    ...
    public function getTarifaFreeDay($fields)
    {
        //sacar cuantos dias hay entre las dos fechas
        ...
        $rangePrice = Tarifa::where('tipo_tarifa', $fields['tipo_tarifa'])
                            ->where('delegacion_id', $fields['delegacion_id'])
                            ->where('desde', '<', $daysDate)
                            ->where('fecha_inicio', '<=', $startDate->format('Y-m-d'))
                            ->where('fecha_fin', '>=', $endDate->format('Y-m-d'))
                            ->orderBy('desde', 'DESC')
                            ->select('desde')
        ...
        //aqui buscaremos los tramos por las fechas
        //caso 1, que fecha coincida con un tramo
        $tarifas = $this->unTramo($fields, $startDate, $endDate, $daysDate, $rangePrice->desde, $fields['tipo_tarifa_id']);
