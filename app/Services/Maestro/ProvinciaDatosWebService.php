...
use Models\Maestro\ProvinciaDatosWeb;

class ProvinciaDatosWebService
{
    public function get($fields)
    {
        $query = new ProvinciaDatosWeb();

        $query = $this->getQuery($fields, $query);
        return $query->with('Provincia','DelegacionDatosWeb')->get();
    }
    ...
    public function edit($data, $id) 
    {
        DB::beginTransaction();
        try {
            $modulo = ProvinciaDatosWeb::find($id);
            $modulo->fill($data);
            $modulo->save();
    ...
    public function getQuery($fields, ProvinciaDatosWeb $query)
    {
        foreach ((new ProvinciaDatosWeb())->getColumnsName() as $column) {
